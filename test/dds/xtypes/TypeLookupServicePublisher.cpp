        ...
        bool& should_be_ignored)
{
    ...
    if (participant_->find_type(discovered_reader_type_name) == nullptr)
    {
        ...
        {
            throw std::runtime_error("Type '" + discovered_reader_type_name + "' is not registered but it should be.");
        }
        if ((!has_type_object && is_registered))
        {
            throw std::runtime_error("Type '" + discovered_reader_type_name + "' is registered but it should not be.");
        }
